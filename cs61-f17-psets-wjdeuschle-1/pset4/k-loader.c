...
int program_load(proc* p, int programnumber,
    ...
    for (int i = 0; i < eh->e_phnum; ++i) {
        if (ph[i].p_type == ELF_PTYPE_LOAD) {
            ...
            int mark_as_readonly = 0;
            // mark as readonly
            if ((ph[i].p_flags & ELF_PFLAG_WRITE) == 0) {
                mark_as_readonly = ELF_PFLAG_READ;
            }
            if (program_load_segment(p, &ph[i], pdata, allocator, mark_as_readonly) < 0) {
                return -1;
            }
        }
    }
...
// program_load_segment(p, ph, src, allocator, mark_as_readonly)
...
//    mark_as_readonly is ELF_PFLAG_READ, otherwise 0
...
static int program_load_segment(proc* p, const elf_program* ph,
    ...
    // mark memory as readonly now
    if (mark_as_readonly == ELF_PFLAG_READ) {
        for (uintptr_t addr = va; addr < end_mem; addr += PAGESIZE) {
